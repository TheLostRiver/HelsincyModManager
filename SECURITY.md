...
普通卸载仍要求所有目标存在、摘要匹配且所需备份可读。安装成功后被外部删除的文件，使用恢复中心
或 CLI 的 `uninstall_missing_targets` 动作：后端先生成只读预览，用户确认后在原有 game/profile
写锁、跨进程准入和取消屏障内复用卸载执行器。

- 必须有可信且 profile 匹配的 manifest、完整 installed 摘要、唯一目标归属及至少一个缺失目标；拒绝目标大小写碰撞。
- 任一仍存在的目标被修改或不可读、所需备份缺失或不可读、游戏正在运行或运行状态未知时拒绝执行。
- 同 profile 有未完成安装恢复记录或尚未清理的重装事务时拒绝执行；不能通过清账绕过恢复证据。
- 确认摘要绑定配置的游戏根、game/profile/Mod、清单、当前目标状态及备份内容；执行时重建并比较，逐项操作前再检查
  目标。该摘要只用于检测漂移，不替代写入准入或用户确认，也不进入日志。
- 缺失且没有备份的条目只从最终原子 manifest 中移除；有备份的条目恢复原文件；仍存在且匹配的无备份
  文件按正常卸载删除。未知内容保持原样，不从当前 Mod 包补文件或猜测原版。
- 写入或清单保存失败时沿用卸载的 best-effort 回滚。回滚先核对目标仍是本次写出的状态，避免覆盖外部
  新内容；原先不存在的目标恢复为不存在。它不新增独立的持久化卸载事务。
- 成功后通过共享 manifest 仓储使库投影失效，并刷新恢复摘要；Task/Audit 继续只记录稳定身份与计数。
...
MHW 普通安装和装备重定向在上述基线之外共用可执行类型政策：

- `nativePC` 内命中 `.bat .cmd .com .dll .exe .jar .js .lnk .msi .ps1 .scr .vbs` 的文件默认不新增安装。
  其中 `nativePC/plugins` 内通过 Windows x64 PE/DLL 结构检查的文件可由用户在原有安装预览中选择。
  判定按 Win32 语义剥掉路径最后一段尾随的 `.` 与空格
  （`x.exe.` 落盘就是 `x.exe`）后再比对，扩展名大小写不敏感。
- 处置是排除对应文件而不是拒绝整包：把贴图转换工具塞进 Mod 包是作者的普遍习惯，包本身没有问题，
  为此让整个 Mod 不可用会重演 #336 的病根。
- adapter 的政策盘点数量进入 `ReplacementAdapterFacts.excluded_file_count` 随清单落盘，并写入
  Audit Log 的 `excluded_file_count` 字段。**审计和日志只出计数，不出路径**。受控安装预览可按下述
  文件处置契约展示经过验证的游戏根相对路径。此计数表示原始政策盘点，不等于最终未包含数；
  插件选择与 `fileEffects` 表达最终包含／保留／移除结果。
- 已知边界：清单是枚举而非「所有可执行类型」的完备刻画（`.hta` `.wsf` `.vbe` 等不在其中）。
  PE 检查只验证结构与架构，不保证插件安全或功能正常。管理器不加载或执行 DLL，不下载随包工具；
  根目录 loader 不纳入本次选择，继续沿用既有前置与安装根规则。

插件的 pending 选择按 game/profile/Mod/revision 保存，盘点 ID 绑定完整文件身份、路径、SHA-256、
大小、包级排除及政策版本。默认勾选不等于授权，新增前须由既有预览确认；CLI／批量先核验准确的
sealed token 与实际计划，再记录该计划批准的选择。调用方不能提交路径、bytes、摘要、确认标志或
应用快照。仓储读写拒绝链接／junction 祖先，原始 Mod 包保持只读。

应用快照随 InstallPlan、manifest、备份与恢复事务保存。失败或取消恢复旧 applied 快照和文件，
pending 意图不被当作已安装事实。无装备绑定的同版本重新应用仅允许明确声明的插件发生变化，
所有非插件文件必须保持；全部 owned 文件均由可信插件事实覆盖且明确排除时，才能移除最后一个
文件并恢复其备份。普通空计划门禁不放宽。

同 revision 的纯装备目标切换有一条安装连续性规则：adapter 明确报告为政策排除、但已由当前 Mod
可信管理的附件，在原包、清单和实际文件一致时保持原位。它们进入完整候选并必须分类为 `Retained`，
不写入附件文件，沿用原有归属及备份引用。接管、归属冲突、文件身份／路径不一致、摘要缺失或内容
变化均不能取得保留资格；原包盘点缺失既有文件也不能伪装成要删除的旧条目。

未安装附件不因目标切换而自动新增；合法插件须明确选择后才可补装，其他 Mod／外部文件不被认领。
受管文件缺失、被改动或来源不可信不能借“排除”选择取得删除许可。普通版本升级和卸载仍按自己的计划
处理。保留结果通过既有事务的 `Retained` 目标及原 manifest 保存，取消、失败和重启恢复规则不变。
排除政策与保留资格只由后端判断；预览 DTO 可提供保留／未包含数量、稳定提示码及文件处置明细中的
受控相对路径，不提供本机游戏／原包／暂存绝对路径，不接收调用方构造的路径或保留证据。

重新应用当前装备目标使用独立意图，后端确定已安装 revision、完整当前目标及文件层级。来源、目标
和版本必须保持，只有实际文件差异才创建重装事务；无变化不签发写入 token、不修改清单。
生成暂存时读取到的原始文件摘要参与 token 与批量摘要，提交时重新核对，不能仅因暂存仍存在就忽略
原包已变化。意图随事务持久化，旧记录默认普通流程；取消、回滚和重启恢复沿用已有保护。

外部 MOD 接管（#286 adopt）是这条基线上唯一**不经安装执行器**的清单写入，其备份、回滚与失败
恢复策略如下：

- 仅允许来源已确认的狩技盒子迁移 Mod（`external_import` / `hunting_box_directory_v1`）。来源由
  持久 logical Mod 记录提供，并在写锁前后复核；普通文件导入、未知来源或其他 adapter 均拒绝，
  不能通过前端传来源标签或路径绕过。其他来源的只读文件状态检查仍可使用。
