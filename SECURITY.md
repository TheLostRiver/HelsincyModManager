...
装备重定向（MHW:I 武器侧，#336 切片③）在上述基线之外还有一条可执行类型拒绝清单：

- 包内命中 `.bat .cmd .com .dll .exe .jar .js .lnk .msi .ps1 .scr .vbs` 的文件**不产出重定向
  动作**，因此不会被写进游戏目录。判定只看路径最后一段，并按 Win32 语义剥掉尾随的 `.` 与空格
  （`x.exe.` 落盘就是 `x.exe`）后再比对，扩展名大小写不敏感。
- 处置是丢弃而不是拒绝整包：把贴图转换工具塞进 Mod 包是作者的普遍习惯，包本身没有问题，
  为此让整个 Mod 不可用会重演 #336 的病根。
- 丢弃必须留痕：数量进 `ReplacementAdapterFacts.excluded_file_count` 随清单落盘，并写入
  Audit Log 的 `excluded_file_count` 字段。**审计和日志只出计数，不出路径**。受控安装预览可按下述
  文件处置契约展示经过验证的游戏根相对路径。
- 已知边界：清单是枚举而非「所有可执行类型」的完备刻画（`.hta` `.wsf` `.vbe` 等不在其中），
  且只作用于重定向计划；普通安装链路目前没有同类清单。`nativePC/wp/` 下的文件不在 Windows
  或 MHW loader 的任何搜索路径上，该清单属纵深防御与目录卫生，不是唯一防线。
...
未安装附件不因目标切换而新增，其他 Mod／外部文件不被认领。普通版本升级和卸载仍按自己的计划
