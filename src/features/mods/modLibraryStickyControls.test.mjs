...
test("sticky controls are an opaque single-column bar fixed above the scroll container", () => {
  ...
  // 吸顶条处于滚动容器(content)之外的 mod-library 第1行，预览面板可占第2行；
...
test("narrow screens keep the mod list usable beneath tall controls", () => {
  ...
  assert.match(
    css,
    /@media\s*\(max-width:\s*860px\)\s*{[\s\S]*?\.mod-library\s*{[\s\S]*?grid-template-rows:\s*auto\s+auto\s+minmax\(320px,\s*1fr\);/,
  );
});
