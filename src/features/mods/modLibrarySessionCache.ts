...
// # 语义：目录缓存与安装状态分别更新
//
// 安装／卸载保留目录页，安装状态由独立的版本化观察和轻量查询更新。导入、删除、元数据
// 编辑或显式刷新仍重新查询目录；状态筛选在写后重查成员与分页。旧展示快照不能代替
// 写入结束后的状态验证，也不充当后端写入授权。
//
// # 淘汰口径：最近写入优先，而不是最近读取优先
//
// 每次读命中后面跟着同 key 的安装状态重新校验写入，所以「最近写入」与
 ...
 * 丢掉全部分页槽位。
 *
 * 用在目录本身变动时：导入、删除或展示元数据编辑。普通安装状态变化保留目录页。
 * 新导入的 Mod 可能命中任何筛选、任何搜索词、
