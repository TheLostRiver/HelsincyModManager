...
test("terminal feedback is published only after durable manifest and recovery refresh", () => {
  ...
  const writeRefreshStart = page.indexOf("const refreshModLibraryAfterWrite");
  const writeRefreshEnd = page.indexOf("const refreshTerminalDurableStatus", writeRefreshStart);
  const writeRefresh = page.slice(writeRefreshStart, writeRefreshEnd);
