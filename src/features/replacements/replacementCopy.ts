...
export const replacementCopy = {
  zh_cn: {
    ...
    panel: {
      ...
      aliasCountTitle: "这些名称对应的装备共用模型，选择任意一项都会一同改变外观。",
      selectedAliasesTitle: "同模型影响范围",
      selectedAliasesCount: (count: number) => `${count} 个名称`,
      selectedAliasesHint: "下列名称与所选项共用模型，替换后外观会一同改变，无法只改变其中一个名称对应的装备。",
      selectedAliasesAria: "受同模型替换影响的装备名称",
  ...
  en: {
    ...
    panel: {
      ...
      aliasCountTitle:
        "These equipment names share model files. Choosing any of them changes their appearances together.",
      selectedAliasesTitle: "Shared model impact",
      selectedAliasesCount: (count: number) => `${count} name${count === 1 ? "" : "s"}`,
      selectedAliasesHint:
        "All names listed here share the selected model. The replacement changes them together; it cannot affect just one name.",
      selectedAliasesAria: "Equipment affected by this shared model replacement",
      targetOccupied: (name: string) =>
  ...
  ja: {
    ...
    panel: {
      ...
      aliasCountTitle: "これらの名称はモデルを共有しているため、どれを選んでも外観が一緒に変わります。",
      selectedAliasesTitle: "共有モデルの影響範囲",
      selectedAliasesCount: (count: number) => `${count} 件の名称`,
      selectedAliasesHint: "以下の名称は選択したモデルを共有しています。置換後はすべての外観が一緒に変わり、一つの名称の装備だけを変更することはできません。",
      selectedAliasesAria: "共有モデルの置換で影響を受ける装備名",
      targetOccupied: (name: string) =>
