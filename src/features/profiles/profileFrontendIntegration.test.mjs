...
test("open folder entry covers the defaulted backup directory and stays disabled in preview", () => {
  ...
  // 托管默认只存在于备份行,由调用处以 canOpenDefaulted 声明,存档行不声明——
  // 即使存档 selection 出现 defaulted 也不渲染一个必报错的入口。
  assert.match(
    source,
    /selection\.status === "valid" \|\| \(canOpenDefaulted && selection\.status === "defaulted"\)/,
  );
  assert.match(source, /canOpenDefaulted=\{false\}/);
  assert.match(source, /^\s+canOpenDefaulted$/m);
