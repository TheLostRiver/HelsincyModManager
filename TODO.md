...
**前置**: 无
**状态**: `design-complete`，尚无实现（2026-09-07 立项，2026-09-08 拍板接入方式与顺序）
...
| 切片 | 内容 | 状态 |
|------|------|------|
| **T21-A** | 失败原因通道 ＋ `unsupported-archive-format` / `not-an-archive` 两档 ＋ 三语 | `ready` |
| **T21-B** | 安全外壳与格式解耦（七条门禁上移），**验收载体是 rar** | `ready`（A 之后） |
| **T21-C** | rar：vendor ＋ FFI ＋ `NOTICE.md` 许可义务 | `ready`（B 之后） |
| **T21-D** | 7z：纯 Rust，复用 B 的外壳 | `ready`（B 之后） |
| **T21-E** | `.tar` 家族 | **已延后**，分析见设计文档 |

#### 三条不可让步的判据

1. **不能把损坏的 zip 说成格式不支持**——否则只是把误导换了个方向
2. **B 的验收不是「重构完成」**，而是 rar 与 zip 共用同一组负测、**一条都不为 rar 重写**；
   要重写就说明外壳没抽干净
...
**对 T21-B 的影响**：这一点已经不是「要新建的能力」，而是「把已落地的正确语义推广到新格式」。
新格式接进外壳时必须复用这套配额，不得各写一份。
...
| T21 压缩包格式扩展 | P2（T21-A 为 P1） | 待开工；**2026-09-08 拍板**：A 档位 → B 安全外壳解耦（rar 作验收载体）→ **C rar（自 vendor UnRAR 7.23 ＋ FFI）** → D 7z；**E tar 家族已延后** | 缺陷面见 #348；上限口径疑点已实测证实并由 #367 单独修复 |
| T22 窗口拖拽批量导入 | P2 | T22-1/2/4 与 L2 内容层预检已实现待评审（`hy/366-drag-drop-import`）；架构决定已定为**不复用 T17**（其候选模型从不保存源路径）。「疑似重复」档查证后确认现在做不了——库里没有内容指纹可比 | 立案 #366；前置 T21-A |
