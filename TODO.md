...
- T21 导入压缩格式支持: `docs/ARCHIVE_FORMAT_SUPPORT_DESIGN.md`
...
**前置**: 无
**状态**: **A / B / C / D 已全部合并**（2026-09-07 立项，2026-09-08 拍板接入方式与顺序并全部落地）；
E 已延后。**RAR4 与 solid 包无自动覆盖**，需真实样本，见设计文档的完成清单
**预估**: A 小 / B 中 / C 中偏大（含 vendor ＋ FFI）/ D 中 / E 已延后
**独立文档**: **已创建** → [导入压缩格式支持设计](docs/ARCHIVE_FORMAT_SUPPORT_DESIGN.md)
**缺陷立案**: #348

> **权威设计在上面那份文档里。** 本条目只保留状态、切片清单与判据，
> 现状核实、威胁模型、判别契约、外壳接口、许可义务与测试矩阵一律不在此重复。

#### 2026-09-08 维护者拍板

1. **rar 做，且自行 vendor 官方 UnRAR 7.23 源码 ＋ 写 FFI**，不用现成 crate
2. **顺序：rar → 7z → 拖拽导入（T22）**
3. **`.tar` 家族整体延后**（含裸 `.tar`）
4. vendor 目录**原样保留 `license.txt`**，许可证不得丢失

#### 切片

| 切片 | 内容 | 状态 |
|------|------|------|
| **T21-A** | 失败原因通道 ＋ `unsupported-archive-format` / `not-an-archive` 两档 ＋ 三语 | **已合并** PR #370（`main@2c4f0f7`） |
| **T21-B** | 安全外壳与格式解耦（七条门禁上移到 `hmm-infra/src/archive_extraction.rs`） | **已合并** PR #371（`main@b588a44`） |
| **T21-C** | rar：vendor ＋ FFI ＋ `NOTICE.md` 许可义务 | **已合并** PR #373（vendor ＋ 布局钉死）/ #374（语料生成器与往返验证）/ #375（接入外壳、加密与分卷分档） |
| **T21-D** | 7z：纯 Rust，复用 B 的外壳 | **已合并** PR #376（含把门禁再抽一层，让「格式自己当驱动方」也复用同一份） |
| **T21-E** | `.tar` 家族 | **已延后**，分析见设计文档 |

#### 三条不可让步的判据

1. **不能把损坏的 zip 说成格式不支持**——否则只是把误导换了个方向
2. **新格式接进来时不得为它重写任何一条负测**——要重写就说明外壳没抽干净。
   **落到 C 身上的硬约束：C 不得改动外壳接口。** 真要改，C 的 PR 必须写明改了什么、
   以及 B 为什么没预见到，不许悄悄改完了事
   （原判据把这条写成「rar 与 zip 共用同一组负测」，那让 B 的完成定义依赖 C、
   B 自己永远收不了尾；B 落地时改为自带一个**形状与 zip 刻意相反**的假格式作第二个接入方，
   见 `archive_extraction.rs` 的 `shared_negative_suite`）
3. **unrar 全程不得碰文件系统**——落盘位置一律由外壳决定。这既是架构一致性，
   也是消除 symlink 绕过那一整类 CVE 的核心缓解

#### 探针已跑完：口子是真的，已单独修掉（#367）

原计划开工前做的探针（构造声明大小远小于实际解压量的 zip）已于 2026-09-08 执行，
**实测确认三道尺寸门禁可被完全绕过、导入还报成功**——声明 100 字节的包实际落盘 8 MiB。

已由 **#367 / PR #368** 单独修复并合并（`main@f055b23`）：声明值预检保留做快速失败，
新增 `copy_entry_with_byte_budget` 按实际写入量判定且判定放在写之前。

**对 C / D 的影响**：B 已把这套配额做进外壳，按实际推入的字节扣、扣在写之前
（`BudgetedSink`）。新格式只负责把字节推给外壳，**不得各写一份配额**——写了就是又开一个同样的口子。
...
**状态**: **T22-1/2/4 已实现待评审**（分支 `hy/366-drag-drop-import`）；T22-3 的架构决定已落地
（结论见下），但**没有**做「后端接收 sealed 多包选择」——见「实际落地与规范的差距」
...
|------|------|------|-----------|
| 可导入 | 正常 Mod 包 | ✅ 勾选 | — |
| 格式不支持 | `.tar.gz` 之类（T21-E 已延后）。**`.rar` / `.7z` 不再属于这一档**——C/D 已落地 | ❌ 不勾选 | **否**——链路根本读不了 |
...
- [x] T22-1：订阅 Tauri 拖放事件、拖入态高亮、L1 容器层分档（复用 T21-A 的档位）
- [x] T22-2：待导入清单浮层——逐行勾选、全选/反选、L2 内容层预检、确认/取消
- [~] T22-3：架构决定已落地（不复用 T17），但**没有**「后端接收 sealed 多包选择」，见下
- [x] T22-4：批量结果 UI（逐行 ✓/✗ ＋ 成功 N / 失败 M 摘要），复用既有档位文案

#### T22-3 的架构决定：不复用 T17，按 T17 的形状新写

开工前查证的结论，推翻了立案时「倾向复用」的倾向：

**T17 的候选模型从不保存源路径。** `ExternalImportCandidate` 记录的是「某个来源里的某个
条目」，materialize 时由来源 adapter 重新定位——这正是它能做 sealed selection 的前提
（选择是可重放的）。而拖拽给到的是一组一次性的绝对路径：窗口关掉、文件被移走，就没有
「重新扫描」这回事。硬塞进去要么让候选模型开始存路径（毁掉那条不变量），要么造一个
假的「可重复扫描的目录来源」。两条都比新写贵。

所以走的是立案时列为下策的那条：**前端逐个起既有的单包导入任务**。代价也照实记：
这确实是第二套批量语义。为了不让它散开，做了三件事——

1. 起任务 / 认领终态 / 串行推进只有一份实现（`modImportDropRunner.ts`），组件不碰这些
2. 结果模型只有一份（`DropImportRun`），逐行状态与摘要都从它派生
3. 逐行档位与失败文案**直接复用导入失败那一套**，不另造词汇

导入串行跑，不并发：并发会同时抢沙箱与 unrar 的进程级锁，收益不明而失败模式很难解释。

#### L2 内容层预检的落地方式

判据是**结构性**的：归档条目名里有没有本游戏的内容目录（`nativePC`），
不是词表——不许出现「文件名带 setup 就拒绝」那类需要事先知道一张表才能工作的规则。

**与 `content_root::resolve_content_root` 复用同一组常量**（目录名、深度上限、
大小写不敏感），只是这里看归档条目名而不是已解包的目录树：拖拽清单不能为了预检去解包。
不另写一份，是因为两处一旦各写各的迟早会对「内容根在哪」给出不同答案（#284 就是这么来的）。
...
|---|---|---|
| `importable` | 勾选 | — |
| `warned`（找不到内容目录） | **不**勾选 | **必须能** |
| `blocked`（加密/分卷/不是压缩包/损坏） | 不勾选 | **不能**——链路物理上读不了 |

`warned` 与 `blocked` 的区别就是这一整档存在的理由：包级否决是错的，我们的判定会错，
错的代价是玩家眼睁睁看着一个好包装不进来。「全选」也把 `warned` 勾上——玩家明确要求了全部。

两条防守写进了测试：zip 逐条目的读取错误一律忽略（这一步只产警示，绝不能把本来可导入的
包变成不可导入，否则预检与真实导入会分叉）；前端认不出的警示码当成没有警示
（后端将来多发新码时，最坏结果应该是「少说一句提示」而不是「一整批突然都默认不勾选」）。

#### 疑似重复档：查证后确认**现在做不了**，不是没做

不是取舍，是缺前提。库里**没有任何能回答「这个包与已有条目是不是同一份内容」的身份**：

| 想用的身份 | 实际情况 |
|---|---|
| 内容指纹 | 普通导入链路根本不算。只有 external import 路径有 `content_fingerprint`，而拖拽从不走那条路径 |
| `package_id` | **等于 task id**，每次导入都是新值，天然不可能相等 |
| 字节大小 | 不入库。projection 的 `size_label` 存的是「导入完成」这个字符串，不是数字 |
| 源压缩包文件名 | 不单独存，只折进 `display_name`；而 `display_name` 的去重是尽力而为的装饰，明确「不参与身份判定」 |

要做这一档，得先给普通导入链路加一份**持久化的内容指纹**并回填既有条目——那是给每个 Mod
新增一条身份，牵涉存储 schema 与迁移，该单独立 issue，不该塞进拖拽导入。
...
|---|---|
| 疑似重复档 | **未做**，前提缺失，见上 |
| 拖到卡片上 = 导入为新版本 | 未做，立案时就划为首版之外 |

因此立案验收里「五行各自落到自己的档位」现在能验到容器层那几档**加上**内容层那一档，
只有「与库中重复」那行验不了。

#### 已做的反向验证（2026-09-08）

容器层与执行引擎 16 条 + 内容层 13 条，共 29 条突变逐个施加、全部转红、0 存活。其中两条是**写测试时当场逼出来的真缺陷**，
不是补写的自证：

- `stopDropImportRun` 只清 `queue` 不清 `currentPath`。调用顺序是「跑完 → advance →
  停止」，而 advance 已经把下一个提升成了 `currentPath`，于是按下停止还会多跑一个
- 进度事件可能早于 `start` 返回 taskId 到达（瞬间跑完的导入必然如此）。丢掉它整批就会
  永远等一个不来的终态

第一轮反向验证本身也暴露了测试的毛病：突变施加后测试不是转红而是**挂住**
（`await` 一个永不 resolve 的 Promise，node --test 默认无超时）。挂住既不算红也不算绿，
那一轮什么都证明不了。已加两道界（`settleWithin` ＋ 每测试 5s 上限）
...
- 反向验证：把内容层判据改成恒真，「不是 Mod」的用例必须退回「可导入」——
  证明这一档由真实判据驱动，而不是恒定命中。**已做**（2026-09-08）：深度上限、
  大小写、分隔符、目录判定、三种格式各自的接线，逐条突变全部转红
...
- 数量上限要有明确档位并给出提示，不能拖 500 个包进来把 UI 卡死 —— **已做**：上限 100，
  超了整批拒绝并报出数量，不截断（截断等于静默丢弃）
...
| T21 压缩包格式扩展 | P2（T21-A 为 P1） | **A / B / C / D 已全部合并**（#370 档位与失败原因通道、#371 安全外壳与格式解耦、#373–#375 rar 自 vendor UnRAR 7.23 ＋ FFI、#376 7z 纯 Rust）；**E tar 家族已延后**。RAR4 与 solid 包**无自动覆盖**，待真实样本 | 缺陷面见 #348（C/D 已落地，**关不关由维护者定**）；上限口径疑点已实测证实并由 #367 单独修复 |
| T22 窗口拖拽批量导入 | P2 | 拖放事件、待导入清单、L1 容器层 ＋ L2 内容层分档、批量串行导入与逐条结果**已实现，待真机验收**（PR #377）；架构决定为**不复用 T17**（其候选模型从不保存源路径）。「疑似重复」档查证后确认现在做不了——库里没有内容指纹可比 | 立案 #366；前置 T21-A |
