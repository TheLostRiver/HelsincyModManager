...
| I18N-04 profiles + 恢复中心三语 | P2 | in-progress | profiles（配置档列表/存档路径/自动备份/恢复对话框/Steam 账户发现）与 install-recovery（恢复中心/全局告警/回滚/诊断导出）全域三语；备份/恢复任务状态与恢复中心 label+guidance 映射按语义码 + copy 传参 getter 分离；页面内联 mock 内容抽独立数据文件（内容不翻译） |
