...
| I18N-02 Mod 库页第一批三语 | P2 | in-review | 库页/工具栏/分页/查询反馈/卡片/右键菜单/快捷面板/导入全量三语（modLibraryCopy + modImportCopy）；选择反馈与导入失败态重构为语义码 + 渲染时取词（reducer/state 层 locale 无关）；查询错误改存稳定 code；mock 数据（Mod 名称/分类）属内容不翻译；批量生命周期/外部导入/详情/重装预览随 I18N-03 |
