...
- [x] T22-2：待导入清单浮层——逐行勾选、全选/反选、确认/取消。**L2 预检没做**，见下
...
| 规范里的东西 | 状态 | 说明 |
|---|---|---|
| L2 内容层预检（包里有没有可识别内容根） | **未做** | 现在只有 L1 容器层。清单的「预检中」中间态已经有了，L2 接上去即可 |
| 「看起来不是本游戏的 Mod」档 | **未做** | 依赖 L2。这一档只警示不否决的口径不变 |
| 疑似重复档 | **未做** | 需要与库中已有条目比内容 |
| 拖到卡片上 = 导入为新版本 | 未做 | 立案时就划为首版之外 |

因此立案验收里「五行各自落到自己的档位」目前只能验到容器层那几档
（正常 zip / rar / 7z / `.exe` / 截断包），「内容不是 Mod」与「与库中重复」两行验不了。
反向验证那条（把内容层判据改成恒真）同样要等 L2 落地。

#### 已做的反向验证（2026-09-08）

16 条突变逐个施加、全部转红、0 存活。其中两条是**写测试时当场逼出来的真缺陷**，
不是补写的自证：
...
| T22 窗口拖拽批量导入 | P2 | T22-1/2/4 已实现待评审（`hy/366-drag-drop-import`）；架构决定已定为**不复用 T17**（其候选模型从不保存源路径，与一次性路径列表不兼容）。L2 内容层预检与「疑似重复」档未做 | 立案 #366；前置 T21-A；预检复用 #354 |
