...
- TypeScript / TSX：超过 500 行开始预警，超过 800 行必须说明为什么不拆。
- Rust：超过 400 行开始预警，超过 700 行必须说明为什么不拆。
- YAML / Workflow：超过 250 行开始预警，超过 400 行必须说明为什么不拆。
- Markdown：超过 300 行开始预警，优先拆到 `docs/` 下的专门文档。

这些阈值不是强行追求小文件，而是提醒协作者不要把总控逻辑越堆越大。
