...
| **286** | **进行中** | 外部 MOD 接管。**切片 1（判定纯逻辑 + 三档徽标投影）与切片 2a（扫描服务：有界并发、可取消、只读）已合并（PR #301）**；尚未接线到 command。**后续切片**：composition 装配 + AppState 暴露 + Tauri command + 结果存储 + 契约登记 → 三语文案与 UI 接线 → 接管 adopt。设计决定与已核实事实都在 issue 评论里 |
...
工作区：**干净**。PR #289 至 #302 的分支本地与远程均已删除；`hy/external-mod-state-scan`（#286 切片 2a，PR #301）为当前唯一开放分支。
...
1. **#286 外部 MOD 接管**（**当前在做**）。已定的口径：按需扫描（不做进每次翻页）、
   ...
   **不引 rayon**）、徽标**三档降级**（tech 完整 / classic·grid 精简 / list 极简）、
   ...
   切片顺序：①纯逻辑 → ②扫描服务（以上两项为 PR #301，已合并）→ ③command 接线 →
   ④三语文案与 UI 接线 → ⑤接管 adopt。
   **注意 ①②尚未被 command 调用**：服务以 `pub` 模块落地（私有模块无人调用会被
   `clippy -D warnings` 判为死代码）。接线时还有一个约束：**结果不能进进度事件**——
   契约明确禁止 payload 携带 `target_path`，所以必须落存储 + 单独 getter
   （与 `get_external_import_preview` 同构）。
