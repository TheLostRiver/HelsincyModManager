...
- 添加独立的任务型首次使用引导 overlay：顶部入口从当前页面启动，覆盖 8 个页面的目录扫描、前置检查、Mod 导入与生命周期、配置档、存档保护等关键功能区；按真实 DOM 锚点引导用户点击导航并在路由变化后继续，条件面板支持稳定兜底目标。引导不会替换既有 UI 或自动触发业务操作，并包含连续的面板迁移、高亮几何和打开、步骤、关闭过渡。
