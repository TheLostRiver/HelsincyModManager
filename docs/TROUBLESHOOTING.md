...
Vite 不自动继承 `.gitignore` 或 ESLint 的排除规则。仓库根的 Rust `target`、嵌套工作树和本地
工具状态若未排除，会在启动时被递归扫描并建立文件监视。一次 Windows 现场观测到超过 25 万个
监视句柄，绝大多数指向非前端目录；这证明额外监视负担，不单独证明所有启动白屏都由它造成。

`vite.config.ts` 仅排除已知的仓库根级非前端目录，在进入子目录之前就停止监视遍历。
`src` / `public` 下同名目录、根级 `.env` 和配置文件仍保留监视；不要用全路径同名目录通配符、
禁用 HMR 或清空缓存来替代边界修复。忽略监视不删除任何目录，也不是文件访问的安全授权。

启动白屏仍需区分 Vite 高负载阶段与之后的等待阶段：先核实 PID 归属并计量 CPU 时间，必要时抓
CPU profile 和首次加载的 Network 时间线。打开 DevTools 后的空 Network 列表没有记录历史请求；
ResourceTiming 已完成条目也不能排除未完成请求。刷新后的正常 DOM 只能作为刷新对照。
...
### 3.5 接管过的 MOD 卸载失败，恢复扫描报「目标变更」

**症状**：对一个通过「接管」（#286 adopt）进入 HMM 管理的 MOD 点卸载，任务以
`install_uninstall_failed:uninstall` 失败，审计 `uninstall_mod -> failure`、`removed_file_count=0`；
卡片/恢复中心显示需要修复，恢复扫描的 issue 是「目标变更（TargetChanged）」。看起来像
「接管把 MOD 弄坏了」。

**根因**：接管之后**文件被改过**（哪个程序改的不重要——2026-09-03 验收时是维护者按步骤给 mrl3
追加了一个字节，但顺序反了：先接管、后改文件）。卸载前会核对清单记录的 `installed_file`
（size/SHA-256）与磁盘现状，不一致就 `TargetStateMismatch` **一个文件都不碰**；恢复扫描按同一
口径报「目标变更」。这与 HMM 正常安装的 MOD 被手改后的行为**完全一致**（契约：「目标摘要不匹配…
都会阻断自动卸载」），对接管条目尤其正确——它们 `backup_ref` 为空、没有可还原的原版，宁可卡住
也不能误删。**不是 bug，维护者拍板不改**（#286 评论 5524997376）。
...
### 4.12 验收命令换了 PowerShell 窗口：变量为空，路径悄悄落到盘根

**症状**：验收步骤说「改动文件后接管应被拒（stale）」，维护者执行了
`Add-Content "$game\one001.mrl3" -Value "x" -NoNewline`，命令无报错，回到应用点接管却
**成功了**。第一反应是「锁内指纹复核失效了」。

**根因**：那条命令在**另一个** PowerShell 窗口里执行，`$game` 在那里从未定义。PowerShell
对未定义变量默认展开为空字符串，`"$game\one001.mrl3"` 变成 `\one001.mrl3`——当前盘的根目录
——于是 `D:\one001.mrl3` 被创建（1 字节），游戏目录里的文件纹丝不动，接管成功是**正确**的。
危险之处在于命令成功、路径合法、没有任何报错，唯一的证据是那个多出来的根目录文件。

**处理**：

1. 写验收步骤时**显式标注**「所有命令在同一窗口跑，变量只定义一次」，把变量定义块放在最前；
   跑 dev 的窗口与跑命令的窗口分开标名（窗口 A / 窗口 B）。
2. 破坏性或前置性的文件改动之后，**先核对再回应用**：`(Get-Item "$game\x").Length` 应给出
   预期的新长度；不对就停。
3. 出现「该被拒的操作成功了」时，先查磁盘事实（目标文件 size/mtime 与预期比、盘根有没有多出
   同名文件），再怀疑后端——2026-09-03 这次后端每一步都是对的。
4. 更稳的写法是给 `Set-StrictMode -Version Latest`，未定义变量会直接报错而不是展开为空。
