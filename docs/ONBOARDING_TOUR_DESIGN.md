...
- 任务型 `hmm.first-run` 首步直接选择语言：首次自动启动为语言页、欢迎页加 18 个核心步骤；核心页面手动启动为语言页加 18 步；局部引导同样先提供语言选择。
- 语言选项统一读取 `localeMeta` 自称名并同时显示。首次无偏好时默认中文，已有偏好（包括跟随系统）不重置；选择即时保存并更新全部引导文案，不改变当前步骤与启动路线。
- 语言选择使用原生单选组，方向键只切换语言，不推进引导。继续、返回、跳过和手动重开沿用既有流程；保留现有内容版本，已完成或跳过的用户不会仅因新增语言首步被强制重新引导。
- 默认跨页流程只覆盖工作台、Mod 管理、存档备份和设置，并从当前核心页面开始旋转；恢复中心、
  分类/标签、备份整理、日志/诊断和“关于”页面只在用户位于该页面手动启动时提供当前页局部引导。
...
- 核心流程覆盖 15 个关键功能区：Steam 扫描、手动目录、启动游戏、前置检查、Mod 导入、狩技盒子迁移与生命周期、
  配置档与存档备份、后台保护。5 个可选页面另保留 13 个局部功能区。引导解释真实入口，但不放行业务控件点击。
- Mod 导入之后单独高亮“迁移第三方 Mod”按钮，说明选择狩技盒子 Mod 库目录、扫描预览与批量导入。
  按钮不可用时回退快捷操作区；迁移说明不自动打开对话框、扫描来源或安装 Mod。保留内容版本，已完成或跳过的用户仍可手动重开查看新增步骤。
...
- 根据目标四周扣除间距与视口留白后的实际空间判断，而不是目标的占屏比例；至少一侧能容纳
  正常宽度和可读高度时才使用浮动面板，窗口扩大后可恢复浮动定位。
- 定位过程中的尺寸限制不能将正文压成细条。标题和底部操作保持可见，长正文在面板内滚动；
  固定面板与无目标的欢迎面板不继承上一浮动位置的可用高度。
...
- 核心关键功能说明由 controls 推进；可选页面保留局部页面介绍；导航任务只允许真实目标点击，并在
  route id 变化后继续。
- 默认四页覆盖 15 个关键功能区，可选五页保留 13 个局部功能区；条件渲染区域使用 primary/fallback anchor。
- 不触发扫描、选择目录或其他业务动作。
