...
- 默认值为 `24`。
- 最小值为 `24`，最大值为 `192`，后端拒绝不在 allowlist 中的值。
- page size 不随窗口宽度或视图模式自动变化，避免切换视图时页码和选择范围跳动。
- 前端持久化 page size 和排序偏好，不持久化当前页、搜索词或选择状态。
- 旧的 `12` 项偏好在读取时归一化为 `24`；原来的 `24 / 48 / 96` 偏好保留。
- 每页容量不改变最多 `100` 项的批量选择上限；选择本页超出上限时沿用明确提示，不自动截取或扩大写入批次。
...
上述为分页首版行为。当前已提供名称、首次导入时间、当前版本大小的六种排序，默认最新导入；使用持久统计
和索引，未知历史值排末尾。数据与交互以 [Mod 库排序](MOD_LIBRARY_SORTING_DESIGN.md) 为准，不依赖 JSON
数组顺序或文件 mtime；下文 `name_asc` 查询仍是有效的显式示例。
...
没有结果或尚未取得结果时不渲染 footer，也不保留占位。有结果但只有一页时，仅在底部保留
紧凑的容量设置与总项数，不显示页码导航，保证调大容量后仍可调小。多页时显示完整分页栏，
页码和翻页按钮作为整体沿 Mod 内容区中线居中，侧边容量设置不影响其中点。
窄窗口中页码导航和容量设置分为两行，页码仍居中，不挤出可视区域：

- page size 使用向上展开的项目自绘 listbox，选项为 `24 / 48 / 96 / 192`，以固定宽度纵向排列。
- 菜单保留方向键、Home/End、Enter/Space、Escape、外部点击关闭和焦点恢复；查询忙碌时不可修改。
...
- 0、1、23、24、25、95、96、97、191、192、193、384、385 条边界。
- 16 条结果、每页 24 的真实浏览器菜单展开；四档完整纵向显示、无横向溢出，单页调大后能调小。
- 旧 12 项偏好按 24 加载，192 项持久化重启、多页导航及刷新期间结果快照保持一致。
...
- 默认每页 24，可选 24/48/96/192，旧 12 项偏好按 24 加载，非法 page size 被后端拒绝。
