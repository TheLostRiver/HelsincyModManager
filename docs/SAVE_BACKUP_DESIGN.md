...
默认策略采用系统文档目录下的专用备份根，而不是应用数据目录、软件安装目录或盘符根目录
（文档目录不可用时回退 `<home>/Documents`，再回退应用数据目录）：

```text
<Documents>/HelsincyModManager/saves/<gameId>/profile-<profileId>/
```
...
- 不放应用数据目录：NSIS 卸载器的「删除应用数据」选项会把它连备份一起清空，而存档备份是
  玩家数据，必须比应用生命周期更长。文档目录同样满足「不在游戏目录内、不侵入盘符根目录」。
...
- 当前使用“默认备份目录”。
- 默认目录的安全标签，例如 `文档/HelsincyModManager/saves`（与后端 `pathLabel` 一致，只给
  玩家能据此定位的相对片段，不暴露完整本机路径）。
- “更改位置”入口。
