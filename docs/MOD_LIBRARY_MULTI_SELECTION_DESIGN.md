...
- 快捷操作区提供持久的“批量选择 / 退出批量选择”入口。
- 普通点击保持单选；`Ctrl + 点击`、`Ctrl + Space`和批量模式下的普通点击都进入统一 toggle 逻辑。
...
“快捷操作”区域在原有操作之后追加带 `ListChecks` 图标和文字的模式按钮，不改变导入、
“选择本页 / 反选本页 / 刷新”分段组和生命周期操作的相对顺序：
...
- 导入 Mod、第三方迁移和导入新版本。
- “选择本页 / 反选本页 / 刷新”原分段组；前两者触发时自动进入批量模式并执行对应操作。
...
- 普通模式不渲染圆形选择指示器，悬停、聚焦和选中时也不显示；单选状态由卡片高亮反馈。
...
安装状态徽标与选择指示器独立：只有“已安装”状态显示安装勾号，“未安装”只显示状态文字。
外部状态已确认安装时同样显示勾号；占用和需留意状态继续使用锁与警示图标。
...
- 批量模式入口追加在原有快捷操作之后，不挤压搜索框，也不重排原按钮。
...
- 进入批量模式时显示选择指示器，退出时移除，不让卡片整体重新排版。
...
- 四种视图仅在批量模式显示固定选择指示器；普通模式悬停、聚焦和选中均不显示圆圈。
...
- 增加 `ListChecks + 批量选择`入口，但不改变原有快捷操作的相对顺序。
- 选择本页、反选本页和刷新保持常驻；前两者从普通模式触发时自动进入批量模式，清空选择仅在批量模式展示。
- 增加全局/本页计数和 100 项上限。
- 落实翻页保留、查询上下文变化清空和通知。
