...
- 内部包的条目名**保留来源目录的原始大小写**。规范化（NFKC + 小写）只用于路径键
  （大小写碰撞检测）与内容指纹，不作为落盘路径——否则内容根会落成 `nativepc`，
  MHW 适配器大小写敏感的 `allowed_install_roots` 匹配不上，整包既装不上也无法比对（#309）。
...
- **保留期**(2026-08-27 修订,见下方「保留期修订」):已执行过导入的批次
  (`completed`/`completed_with_errors`/`failed`/`cancelled`)**永久保留**,不按数量、
  也不按时间删除;只扫描未导入的批次(`import_status = pending`)只按数量保留最近 `50` 个,
  不再有时间过期;`running` 永不清理。清理在启动期 `recover_interrupted_batches()` 之后执行一次,尽力而为、
...
- 保留期:`running` 永不删除;imported 批次在任意数量下都不删除(含远超旧 `50` 上限的场景),
  且其候选/结果行一并保留;超出数量上限的 scan-only 删除后候选/selection/结果无孤儿行;
  清理不依赖时钟——时钟不可用时仍照常执行。
...
- selection 使用独立集中预算：最多 `10,000` 个候选、`1,000,000` 个文件、`64 GiB` 源字节和 `64 GiB`
  物化字节。单项物化预算见下方「单项预算修订」；放宽任何值必须单独 review。
...
Slice 5 原定「已导入批次保留最近 `50` 个、只扫描批次 `10` 个且不超过 `7` 天」。实际使用后
推翻了这个口径：**从第三方盒子迁移是低频动作**——一个玩家可能一生只做几次，而「导入了哪些、
成功/失败了哪些」是长期可追溯事实，没有理由到期作废。

| 类别 | 旧 | 新 | 理由 |
|---|---|---|---|
| 已导入（终态 `import_status`） | 最近 `50` 个 | **永久保留** | 低频产生、体量小（只有实际候选数的行）、追溯价值最高 |
| 只扫描（`pending`） | `10` 个 + `7` 天 | 最近 `50` 个，**无时间过期** | 不含导入事实，但打开弹窗扫一次就产生一个，且各可带最多 `10,000` 行候选 |
| `running` | 永不清理 | 永不清理 | 结果仍在增量落库 |

两点实现口径：

- 已导入批次不再是「上限很大」而是**结构上跳过清理**：`ExternalImportBatchRetentionRequest`
  刻意只保留 `max_scan_only_batches` 一个字段，不给调用方留下删除导入事实的入口。
- 去掉时间过期后**清理不再依赖时钟**：`prune_batch_history` 不再读 `now`，时钟不可用也照常
  执行，不会因为读不到时间就把启动期清理整体跳过。

保留 scan-only 数量上限的原因是它与已导入批次的体量特征不同：它不含任何导入事实，却可能各带
最多 `10,000` 行候选。若将来要连它也永久保留，需要先复议候选行的存储成本。

### 单项预算修订（2026-08-27）

Slice 1 的单项预算是在没有真实来源数据的情况下拍的。维护者用真实狩技盒子库做了一次脱敏
人工探测（16 个候选、7,749 文件、0.70 GiB），结果推翻了原来的比例假设：

| 预算 | 旧值 | 新值 | 实测峰值占用（旧值口径） |
|---|---|---|---|
| 单项文件数 | `16,384` | `65,536` | 单个普通 Mod 就用掉 **44.8%**（7,339 文件） |
| 单文件字节 | `1 GiB` | `4 GiB` | 最大单文件 `100.75 MiB`，仅 **10%** |
| 单项总字节 | `4 GiB` | `16 GiB` | 字节最大的候选 `241 MiB`，仅 **5.9%** |
| 目录深度 | `64` | `64`（不变） | 实测最大 7 |

关键结论是**文件数预算比字节预算紧约 7.6 倍**（全库峰值口径：文件占用峰值 44.8% vs 字节占用
峰值 5.9%）。注意这两个峰值来自**两个不同的候选**：文件最多的那个（7,339 文件）自身只占字节
预算的 `3.45%`，对它而言文件预算比字节预算紧 **13 倍**；字节最大的那个（`241 MiB`）只有 33 个
文件。两种形态都存在，但撞上限的永远是文件数那一侧。

MHW:I 的松散文件模式（实测 `.tex` 99 个/450 MiB、`.mod3` 466 个/114 MiB）决定了大型材质整合包会
先撞文件数上限，而不是任何字节上限。单文件上限抬到 `4 GiB` 是为了容纳重打包 `chunkG*.bin` 式的
分发（游戏自带 chunk 就是数 GB 级）；单项总量必须同步抬高，否则单文件上限形同虚设。

这三个值与 `hmm-infra` 的 `DEFAULT_ZIP_MAX_ENTRIES` / `DEFAULT_ZIP_MAX_SINGLE_FILE_BYTES` /
`DEFAULT_ZIP_MAX_TOTAL_UNCOMPRESSED_BYTES` **必须保持一致**——同一个 Mod 不能走第三方迁移能进、
打包成 zip 手动导入反而被拒。修改任何一侧都要同时改另一侧。

批次级预算（`10,000` 候选 / `1,000,000` 文件 / `64 GiB`）本次不动。按实测均值（每个 Mod 约 484 个
文件、46.9 MiB），批次文件数上限在约 `2,066` 个候选处先于候选数上限触发，字节上限在约 `1,400` 个
候选处触发——即广告中的 `10,000` 候选对真实 Mod 而言不可达。这对现实规模（≤1,000 个 Mod）不构成
约束，但如果将来要兑现 `10,000` 这个数字，需要另行复议批次级预算。
