...
- `rollback_install`、`reconcile_reinstall` 与 `uninstall_missing_targets` 事件只记录 `task_id`、`game_id`、`mod_id`、`profile_id`、`remove_file_count`、`restore_file_count` 和 `backup_count` 等短 id/计数；`reconcile_reinstall` 的计数既可表示 post-commit cleanup，也可表示受控恢复到 pre-reinstall 状态的 remove/restore/snapshot cleanup 数量。缺失目标卸载记录实际删除/恢复数，无备份且已缺失的条目不计入文件写动作。事件不记录预览 token、完整本地路径、backup/snapshot ref 或 root、manifest 正文、sandbox/cache 路径或第三方 Mod 内容。
