...
- `hmm-app` 已提供最小 `AuditLogDiagnosticsExportService`，可通过 `AuditLogReader` 读取最近 N 条已校验审计事件并写入受控 `audit-log-diagnostics.json` 诊断包，同时为该导出动作写入最小 Audit Log 事件；该服务尚未暴露为 Tauri command。
