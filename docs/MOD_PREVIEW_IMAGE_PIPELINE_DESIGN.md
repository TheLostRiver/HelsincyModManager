...
- scanner 先递归发现名称大小写不敏感等于 `nativePC` 的目录，但发现后不进入该目录。
- 只把 `nativePC` 父目录中的直接普通图片文件作为候选；不扫描 `nativePC` 内部，也不递归扫描父目录
  下的其他图片目录。
- 带外层包装目录的包以实际 `nativePC` 所在目录为候选根，而不是强制使用 sandbox 根。
