...
当前尚未定义 UI/command 设置入口、定时后台任务或手动清理入口；这些属于后续缓存生命周期治理。已有的 `prune_to_size_limit`、settings 读取和默认导入后维护只属于后端生命周期能力，不改变前后端契约。
...
- 支持 UI/command 设置入口、定时或手动触发的空间清理，以及更完整的保留策略。
- 支持定时后台缓存维护任务。
