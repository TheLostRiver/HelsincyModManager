...
当前尚未定义 UI/command 设置入口、手动清理入口或按时间保留策略；这些属于后续缓存生命周期治理。已有的 `prune_to_size_limit`、settings 读取、默认导入后维护和定时后端维护只属于后端生命周期能力，不改变前后端契约。
...
- 支持 UI/command 设置入口、手动触发的空间清理，以及更完整的保留策略。
