...
- scanner 先递归发现名称大小写不敏感等于 `nativePC` 的目录，但发现后不进入该目录；目录遍历带有显式
  深度上限，超过上限时按未找到 `nativePC` 处理，不会无界递归。
- 递归会收集所有命中的 `nativePC` 父目录并去重作为候选根集合；候选图片可以来自多个候选根，最终
  排序仍保持全局稳定。
- 只把每个候选根中的直接普通图片文件作为候选；不扫描 `nativePC` 内部，也不递归扫描候选根下的
  其他图片目录。
- 带外层包装目录的包以实际 `nativePC` 所在目录为候选根，而不是强制使用 sandbox 根；包内存在多个
  `nativePC`（例如多层包装或多子包）时对应多个候选根。
- 如果包中不存在 `nativePC`，为了兼容根目录 DLL 等 Mod，只读取 sandbox 根目录中的直接图片，仍不
  递归扫描任意子目录。
- 优先匹配常见名称，例如 `preview`、`cover`、`poster`、`thumbnail`、`image`。
- 支持常见扩展名，例如 `.png`、`.jpg`、`.jpeg`、`.webp`。
- 文件名不要求英文或固定格式；中文、数字和普通文件系统允许的符号按同一规则参与候选排序。
...
候选扫描不应读取真实游戏目录，也不应读取原始压缩包外的路径。图片位于 `nativePC` 内部时，即使
扩展名和 magic bytes 都合法，也不属于预览图候选。
...
- 根级和外层包装目录中的 `nativePC` 同级图片可以被发现。
- `nativePC` 内部图片、同级子目录中的图片以及非候选根图片不会被发现。
- `nativePC` 名称大小写不敏感，中文、数字、符号文件名和多图排序保持稳定。
- 不含 `nativePC` 的根目录 Mod 只读取 sandbox 根级图片，不递归扫描子目录。
- 正常 PNG 生成缩略图。
- 正常 JPEG 生成缩略图。
- 正常 WebP 生成缩略图。
- 没有预览图时返回 fallback。
