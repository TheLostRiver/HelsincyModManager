...
路线图没有 `ready` 切片时，不自拟新功能。汇总路线图中的所有非完成状态，包括
`implemented`、待 review、`blocked`、需要人工验收以及等待维护者决策的切片，并同时列出
required CI、review 和合并门禁残留。
只有路线图范围全部完成、不存在上述任何遗留项，且没有 required CI、review、合并门禁或验收
工作残留时，才能把项目目标标记为完成。
