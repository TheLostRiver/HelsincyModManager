...
2. 唯一的队列例外是 QG-01：它优先开 PR，但如果只因必需的人工治理 review 等待，保留该 PR，
   继续从当前 main 为 T13-00 创建独立分支；在 QG-01 合并前继续手动运行前端测试和 clippy。
...
9. 治理、安全门禁、CI、workflow、policy、AGENTS 或核心安全文档的变更已经取得项目要求的人工 review；
   缺少时只保留 PR，不使用 admin 绕过。
10. 高风险 Windows 安装态验收若是该 task 的完成定义，必须在 disposable VM/一次性账户完成并 cleanup。

合并动作：
1. 先尝试普通合并。
2. 只有上述 10 项全部满足、普通合并仅被已知的分支保护批准规则阻挡，且该 PR 不属于必须人工 review
   的治理/安全门禁变更时，才允许使用 gh pr merge --admin。
