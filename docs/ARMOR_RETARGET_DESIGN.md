...
>
> **唯一的例外是 `.tex`：槽位目录内的贴图必须留在原路径。** 防具侧零二进制改写，MRL3 按
> **路径**引用贴图，搬走就是静默断链——计划成功、装完游戏里贴图没了。判据是扩展名，这是
> 格式事实（MRL3 是材质/贴图表，按路径引用的只有 `.tex`），不是「记录某个 Mod 的文件类型」
> 那种词表。
>
> 注意 #336 正文由真机实验 B 推断出的「**被 MRL3 引用的**文件留在原路径」不是可靠口径：
> 那次观测里「留下的文件」与「被引用的文件」恰好重合，但两者并无因果关系。可靠的判据是
> 扩展名——`.tex` 不搬，其余照搬。
>
> **武器侧不适用**：武器走策略 B，`.tex` 照搬并同步改写 MRL3 引用，因此源槽位可以完全释放。
>
