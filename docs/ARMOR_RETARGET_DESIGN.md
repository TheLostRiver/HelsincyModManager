...
> **路径结构事实（#342 更正）**：上一版写的「`<slot>` 之后是固定结构目录 `arm/mod`，部位不作为
> 独立目录段出现，retarget 永远只改写 `<slot>` 这一段」**已被真机数据证伪**。真实套装在
> `<slot>` 之后就是部位目录：
>
> ```text
> nativePC/pl/f_equip/pl078_0000/{arm,body,helm,leg,wst}/mod/f_<部位>078_0000.<ext>
> ```
>
> 而且**文件名必须跟着改写**：真机实验 A（重定向前后对游戏目录做全量快照，逐文件比对
> path + size + SHA-256）观测到 `f_arm078_0000.mod3` 落成 `f_arm123_0000.mod3`，哈希证明
> 字节完全相同、只有路径变了。只改 `<slot>` 段会装出「目录对、文件名错」的结果——比打不开
> 更糟，因为它看起来成功了。
>
> **但不要把这段更正读成「部位词表是 `arm/body/helm/leg/wst`」。** 补一张词表只是把同一个
> 错误换个地方犯：作者自建的子目录、没见过的扩展名会再次触发「形态不认识 → 否决整包」，
> 而 Mod 作者的目录结构不可能被穷举。#342 采用的是**结构规则**：
>
> ```text
> 作用范围 = nativePC/pl/<equip>/<slot>/ 之下的一切，任意深度、任意扩展名、不认识部位
> 改写     = 在该文件的整条相对路径上，把槽位编号段 <3位>_<4位> 换成目标槽位的编号段
> 其余     = 一个字节都不碰
> ```
>
> 这条规则不需要预先知道任何部位名或扩展名，因此新 Mod 的任何目录布局都不会让它失效。
>
> **当前默认规则更正：** 槽位内 `.tex` 与其他资源一起迁移，MRL3 中对包内迁移贴图的引用同步更新。
> 只移动贴图会断开引用，只保留贴图又会继续覆盖原装备，因此两者必须同计划处理。
> 武器、防具共用[装备资源保留策略](MHW_RETARGET_RESOURCE_STRATEGY.md)；旧落点由 manifest 管理的
> 重装事务清理，原包只读，材质改写仅发生在 staging 或只读预览的内存中。
>
> **catalog 的 `metadata.parts` 与真实目录段是两套词汇**，不可混用：catalog 用
> `head/body/arms/waist/legs`，磁盘上是 `arm/body/helm/leg/wst`，四个对不上。`parts` 仍然
> 只是展示用的逻辑标签，**任何拿它拼路径的实现都是错的**。
...
- 把路径解析成上述段。
- 在已确认的槽位子树内，将完整源槽位 token（例如 `121_0000`）替换为目标 token；其余字符保持原样。
- 目录与文件名同时按 `ArmorResourcePath` 的现有 token 规则映射，不将任意短数字作为替换依据。

例如 `f_body121_0000.mod3` 的槽位编号随目标迁移；不含完整源 token 的作者命名不改。
贴图使用同一最终路径映射，材质引用按实际包内贴图对应关系更新。
...
- 原包保持只读；一般资源保留字节，MRL3 仅更新指向包内迁移贴图的固定字段。
...
- 已为 MHW:I 建立 catalog：AR1 的 `mhw-armor-v1` 四条手工 seed 已于 2026-08-21 由 AR6
  ...
  2026-08-24 `mhw-armor-v3` 补齐余下 5 条活动/联动装缺失的名称（权利人 Capcom，经 kiranico
  转录对照）：4 条补 en/ja 展示名，pl057_0010 补 ja 展示名、其官方英文名与女版重名按治理
  规则记为 en alias——最终 268 条三语展示名 + 1 条 zh_cn/ja 展示名带 en alias，并新增键集
  完备性防回归测试；重签记录见 GAME_TERMINOLOGY_SIGNOFF.md。
