...
- 把路径解析成上述段。
- 在已确认的槽位子树内改写装备根和符合编号边界的源槽位 token；作者命名中的无关字符保持原样。
- 目录与文件名同时按 `ArmorResourcePath` 的编号规则映射，不能用任意数字的全局字符串替换。

例如 `f_body121_0000.mod3` 的槽位编号随目标迁移；更长数字、其他槽位编号或不符合边界的字符不猜测替换。
