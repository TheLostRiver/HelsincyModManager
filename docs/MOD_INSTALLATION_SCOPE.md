...
普通安装、首次重定向安装、卸载、真正重装和恢复执行器共用带投影跟踪的清单仓储。
清单写入前后标记该安装命名空间的查询投影失效，下一次库查询重建状态；不能仅修正当前页卡片，
否则后端按旧状态筛选分页时仍会遗漏 Mod。正常缓存复用与服务端分页保持不变。
同一数据库的 GUI 与批量临时 runtime 共用查询重建／事实写入协调，防止并行查询覆盖失效标记；
不同数据根保持独立，已释放 runtime 的协调对象不常驻缓存。

批量使用同一份清单和数据库。安装完成即进入“已安装”，卸载完成即进入“未安装”；部分失败
逐项保留实际状态，不把整个选择集合乐观标为成功。前端在单项终态及批量请求结束时失效页面缓存，
与存档配置档的切换无关。相关回归先预热查询，再在同一桌面 runtime 中执行并查询，不以重启掩盖问题。
