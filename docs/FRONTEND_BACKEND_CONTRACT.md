...
- Mod 库前端可以在 `get_install_manifest_status` 摘要刷新后调用 `scan_install_recovery`。前端应把 `completed` 映射为可展示的 `installed`，把 `repair_required` / `unknown` 作为不安全状态展示并阻断安装/卸载；扫描失败时应保留 manifest 摘要，不应回退为 mock 安装事实或从任务内存态推断已安装。
