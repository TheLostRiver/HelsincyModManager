...
```powershell
node --test "src/features/install-config/*.test.mjs" "src/features/install-plugins/*.test.mjs" src/features/replacements/retargetWorkspaceBehavior.test.mjs
```
...
自动验证不执行真实游戏文件操作，不能代替原生窗口与游戏内验收。
...
共享工作区的展开/收起应覆盖宽屏列宽过渡与窄屏切换、快速反向操作和减少动态效果；离场面板立即
退出键盘导航，动画不得重挂内容或影响搜索、勾选、滚动和任务状态。跨窄窗断点后仅可见区域参与交互。
...
卡片打开延迟为 100ms，详情未返回时先显示库中已有的名称、作者、版本和分类。浏览器需验证快速
划过不请求、四种视图的开关关闭后不弹出且不查询、关闭中途请求的迟到响应无效、刷新后偏好保留，
以及 localStorage 不可用时仍可开关。开关默认关闭，仅保存值为 true 时开启；验证首次使用不查询、
开启和关闭两种选择都能在重启后保留。开关位于分类标签旁，三语可访问名称和按下状态应匹配。
毛玻璃检查浅色／深色主题、长文本和窄屏；背景模糊不作用于文字，减少透明度或高对比度模式使用实色。
