...
附带规则与脱敏断言；`modDetailDialog.test.mjs` 覆盖空值往返；`replacementDetailUi.test.mjs` 改为
断言面板已委派给错误文案模块；Rust 侧新增
`hmm-games-mhw/tests/weapon_error_code_contract.rs` 冻结码表，与前端 `Record` 穷尽映射互为闸门。
