...
1. **压缩包格式扩展**（#348）：目前只支持 zip。按「先说清楚 → 抽安全外壳 → rar → 7z」推进；
   **`.tar` 家族本轮延后**。前两步已合并——`.rar` 不再被报成「请检查压缩包后重试」，
   解包门禁也已与格式解耦；**剩下的是 rar 与 7z 本身**。
   rar 的 UnRAR 许可已核对原文（禁的是压缩器，解压不受限、无商业限制），
   2026-09-08 拍板**自行 vendor 官方 7.23 源码 ＋ 写 FFI**，不用现成 crate。
2. **拖拽批量导入**（#366）：把单个或多个 Mod 包拖进窗口即可批量导入。目前唯一入口是文件对话框且一次只能
   选一个包。**落点是确认清单，不是导入动作**——逐个可取消勾选，格式不支持 / 不是压缩包 / 损坏 /
   「看起来不是本游戏的 Mod」（例如拖进来一个软件安装包）各自有明确档位与提示。
