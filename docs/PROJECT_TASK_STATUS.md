...
跨进程写入 admission 已完成工程实现、本地完整验证和 findings-first 全 diff 审查，仍等待 Ubuntu
required CI 与 disposable Windows 多进程候选门禁；Production CLI command-level admission 与开放仍未完成。
...
| CLI 自动化入口 | CLI-2C 已实现；CLI-3A 本地验证通过/待 CI 与候选验收 | 已有只读 game/install/backup/diagnostics 命令，以及仅 Sandbox 的单项 install/uninstall/reinstall/recovery apply；5 分钟 token、双确认、写锁内重验、取消、失败恢复与 Production 双层拒绝已覆盖。CLI-3A 已接入 game/save/background 三类跨进程 admission、稳定错误码和共享 GUI/CLI/worker composition；Production command-level 写入仍未开放 |
...
和固定 worker；本地完整验证与 findings-first 全 diff 审查已通过，当前等待 Ubuntu required CI 与
disposable Windows gate，尚未标记为 certified。
...
- Unix file-lock 与 capability-relative no-follow 分支未在 Windows 本机声称通过，必须由 Ubuntu required CI 实际验证；disposable Windows 多进程/安装态后台注册仍是候选人工 gate。
...
6. CLI-3A 的本地完整 verify 与 findings-first review 已完成；继续等待 Ubuntu required CI 与
   disposable Windows 多进程 gate。通过后再进入 CLI-3B，按 command 复核 capability、token、Audit、
   锁内事实和 Windows 验收。Production 写入仍不能仅凭跨进程 guard 绕过这些门禁。
