...
| 3840x2160 @ 50% / 33% / 25% 浏览器缩放 | 4K 低缩放工作流 | 外壳仍居中且受上限约束，卡片、骨架屏、操作区保持同一栅格节奏 |

## 缩放韧性

4K 屏幕配合 50%、33%、25% 等浏览器低缩放比例时，浏览器会暴露远大于常规桌面的 CSS 视口。这个场景不应被当作某个固定缩放比例的特殊皮肤处理，而应由同一套外壳上限、布局 token 和网格规则自然覆盖。

50%、33%、25% 都是必测缩放档，不允许只验证其中一个比例后推断其他比例也可用。
...
- 低缩放比例越极端，越应优先保证结构稳定和内容可扫描，允许外壳外侧保留空白，不为了铺满屏幕而破坏模块比例。
...
- 在等效 4K 50%、33%、25% 低缩放视口下，`.app-shell` 仍居中且宽度不超过设计上限。
