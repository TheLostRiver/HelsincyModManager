...
- 设置页「界面偏好」与新手引导第一步提供语言切换（设置页与主题入口同区，不做顶栏快捷切换），
...
- 安装器（NSIS/WiX）与系统对话框文案独立评估，不在本单元范围。现状（2026-09-18）：
  NSIS 安装器已支持中英双语（`bundle.windows.nsis.languages`，见
  `src-tauri/tauri.windows.conf.json`），中文系统走简体中文、其余走英文；WiX 路线
  当前配置不产出 MSI，未做。
