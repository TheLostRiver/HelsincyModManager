...
1. Crash/recovery 扫描后续：在独立恢复中心补充受控人工处理路径，但仍不能绕过 manifest、backup、Audit Log 和恢复扫描事实。
