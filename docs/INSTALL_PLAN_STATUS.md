...
- 恢复扫描：只读 `scan_install_recovery` 摘要已能检测 `completed`、`repair_required`、`unknown` 和 `not_installed`，也支持空 `modIds` 扫描当前 profile manifest 内全部已知托管 Mod；Mod 库加载后已会消费该摘要并展示人工处理提示，Dashboard 入口也已展示 profile 级健康摘要。但尚未实现全局后台启动告警、`rollback_required` rich 状态、自动回滚/恢复执行或独立恢复中心入口。
...
1. Crash/recovery 扫描后续：把当前 Mod 库和 Dashboard 的只读 recovery scan 摘要扩展到独立恢复中心，并给出受控恢复/人工处理路径。
