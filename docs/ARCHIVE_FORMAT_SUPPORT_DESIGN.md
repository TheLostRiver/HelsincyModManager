...
用 `sevenz-rust2` 0.22（**Apache-2.0**，纯 Rust）。

#### 「B 的接口若做对了，这里是复用而非重来」——一半对

**门禁确实是复用的**，但**接口不得不扩充一次**，如实记下来。

`sevenz-rust2` 只提供 `for_each_entries(|entry, reader| …)`，`ArchiveReader` 与
`BlockDecoder` 都只有回调版，**没有任何 pull 接口**（逐个查过）。也就是说
**它要当驱动方**，而 B 定的 `ArchiveSource` 假定驱动方是外壳。两个驱动撞在一起。

解法是把门禁从循环里再抽一层：`ArchiveGate` 持有跨条目状态，对外暴露
`accept_entry(header, produce)`；`extract_archive` 退化成它之上的一层薄驱动。于是

- pull 型（zip / rar）：外壳跑循环，逐条调 `accept_entry`
- push 型（7z）：适配器在**别人的**回调里调 `accept_entry`

**七条门禁仍然只有一份实现**，共享负测整组跑在 7z 上、一条未重写。
`ArchiveSource` 那条路径一个字节没改，C3 的 rar 与既有 zip 测试全部原样通过。

> **这是 B 没预见到的形态。** B 当初只见过 pull 型格式，把「外壳驱动」写进了接口假设。
> 记在这里而不是悄悄改掉——与 C 的硬约束同一个道理。

#### 与 rar 的两处形态差异

| | rar | 7z |
| --- | --- | --- |
| 打开方式 | **只认路径**，必须先把 reader 落一份盘 | 直接吃 `Read + Seek`，**零拷贝** |
| 驱动方向 | 外壳驱动（实现 `ArchiveSource`） | **格式驱动**（在其回调里调门禁） |

因此判别顺序定为 **zip → 7z → rar**：要落盘的排最后，否则导入一个 7z 会白拷一遍整包。

#### 编解码器只开纯 Rust 的

`default-features = false`，**不开 `compress`**（「不做压缩」是非目标，同时少一半代码面），
**也不开 `zstd`**——它是这个 crate 里唯一会拉进 C 实现（`zstd-sys`）的编解码器，
而 7z 用 zstd 需要非标准的 7-Zip 构建，实际几乎见不到。为一个罕见格式把 C 解析器放回
不可信输入路径上，与我们为 unrar 那一整套论证自相矛盾。

其余全是纯 Rust：`bzip2` 走 `libbz2-rs-sys`（名字带 `sys`，实现是 Rust）、
`brotli` / `lz4_flex` / `ppmd-rust` / `flate2` 亦然。已用 `cargo tree` 核对过。

#### symlink 有**两种**表达，都要认

- Unix 侧：属性位 `0x8000` 表示高 16 位是 Unix mode，再看 `S_IFLNK`（`0xA000`）
- Windows 侧：重解析点位 `0x0400`

只认一条等于对另一半平台打的包不设防。两条各有一个用例。

#### 语料可以用同一个 crate 造

与 rar 不同，**写 7z 没有许可障碍**，所以 `compress` 特性只在 `dev-dependencies`
里打开，语料直接用它的 writer 生成，不必手搓容器格式。加密包（内容加密与头加密两种）
也因此都有真实语料覆盖。

> 落地时踩到的坑：`ArchiveEntry` 的属性字段有一个**单独的存在标志**
> `has_windows_attributes`。不置它，写出来的包根本不带属性，symlink 用例会
> **静默变成普通文件而通过**——最难发现的那类假绿。

#### 反向验证顺带量出了一条纵深防御

单文件限额在 7z 侧由**两道彼此独立的门**把着：`accept_entry` 里的**声明值预检**，
与 `BudgetedSink` 里的**字节流配额**。突变实验：
...
也就是说单条突变测不出它们——这不是覆盖不足，而是**冗余防御在突变测试下的正常长相**。
rar 那边只有字节流配额承重（它声明「未知」），所以拆一道就红。记下来，
免得日后有人看到「拆了没红」就以为某道门是死代码而删掉它。

**如实说明**：7z 的「声明值说谎」这一情形没有真实语料——写入器是我们自己，它诚实。
该情形的覆盖来自 zip（#367 的说谎语料）与形状相反的假格式，走的是**同一份**
`BudgetedSink`，但**没有经由 7z 适配器测过**。
