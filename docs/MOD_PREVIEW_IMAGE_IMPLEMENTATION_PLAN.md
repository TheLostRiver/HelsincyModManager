...
- 当前已落地 `start_import_mod_task` 最小入口：只校验 archive 路径，通过最小 `TaskManager` 登记 queued 的 `mod_import` 任务，返回 `TaskStartedDto { taskId, kind, status }`，并发送 `mod_import.queued` 任务事件；安全解压、后续阶段事件、`ModImportAnalysisService` 接线和 library/detail 查询仍是后续任务。
