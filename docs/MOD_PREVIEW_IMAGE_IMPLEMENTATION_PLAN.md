...
| `AuditLogDiagnosticsExportService` | 后端 app 层基础已落地 | 通过 `AuditLogReader` 读取最近 N 条已校验审计事件，写入受控 `audit-log-diagnostics.json` 诊断包，并为该导出动作写入最小 Audit Log 事件。该服务尚未暴露为 Tauri command，不改变当前预览图诊断 zip，也不导出原始日志、未脱敏路径、第三方 Mod 内容或缩略图 URL。 |
