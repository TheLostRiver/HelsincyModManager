...
> **2026-09-07 口径修正：本文件已不再是逐条任务状态的真源。**
> 2026-08-28 起主线转为**真实 Mod 包驱动的 issue 队列**，逐条状态以 GitHub issue 为准。
> 本文件继续承担两件事：① 保留编号切片队列（`T##` / `WR-##` / `SAVE-##` / `CLI-##`）的历史与剩余项；
> ② **「固定范围」「硬门禁」「每个纵向切片的完成定义」「停止条件」四节继续对所有工作生效**，
> 包括 issue 驱动的工作。上一轮把每个 issue 镜像进文档的做法没有维持住——本文件、
> [路线图](ROADMAP.md) 与 [任务总纲](../TODO.md) 曾集体停在 8 月下旬，不再重复。
>
> 上一版基线：`main@5897fbc`（2026-08-07）；Gate C、WR-04 Gate D 与 SAVE-02 安装态后台保护均已完成受控验收。

## 固定范围

- 平台：Windows。
- 游戏：MHW:I。
- 首要目标（2026-09-07 起）：**让既有能力对玩家库里的真实 Mod 包真正成立**——导入、内容根识别、
  重定向判定与安装写入四道关口的命中率，而不是新增能力。
- 并行目标：导入入口扩展（T21 压缩格式、T22 拖拽批量导入）。
- 已达成的前序首要目标：单项与批量的安装/卸载/真正重装（Gate A/C `certified`）。
...
- CAT-01、WR-01、WR-02A、WR-03A、WR-03B 与 WR-04 已完成；WR-04 Windows Gate D 为
  ...
  retention/备份中心均已 `certified`。
- Production catalog 已开放：WR-05 于 2026-08-22 翻转门禁并退役 developer seed，`game_terminology`
  许可签核于 2026-08-23 完成；防具 catalog 已于 2026-09-07 随 #356 扩到 529 条（按模型变体分片）
  并完成 `mhw-armor-v4` 重签。发版前置仍未清：全量 catalog 的 Sandbox Gate 复验。
- **I18N-01 至 I18N-08：`completed`**，界面三语主线已全部交付（PR #242–#247）。
- **2026-08-28 起的 issue 驱动阶段**：导入与安装可诊断性、外部 MOD 状态与接管（#286 全链路）、
  Mod 存储目录可配置与移动式导入（#275）、重定向对真实包的适配（#337/#342/#345/#292/#346/#349/#356/#363）、
  安装前包内容干预（#354 D1–D4）均已合并。**逐条状态见 GitHub issue**，
  阶段结论见 [路线图 · 当前执行焦点](ROADMAP.md)。
...
2026-09-07 新增的两项目标，与重定向主线正交，可并行。**完整范围、已核实的现状、切片、验收与硬边界
见 [任务总纲](../TODO.md) 的 T21 / T22**；此处只登记队列状态与本文件特有的门禁映射。

### T21：压缩包格式扩展（zip → rar → 7z）

状态：T21-A `ready`；T21-B `ready`（A 之后）；T21-C / T21-D `ready`（B 之后）；
T21-E **`out_of_scope`（本轮）**。

**2026-09-08 维护者拍板：rar 做且自行 vendor 官方源码；顺序 rar → 7z → 拖拽导入（T22）；
`.tar` 家族整体延后（含裸 `.tar`）。** 停止条件里的「安全取舍未拍板」已解除。

- **T21-A**「不支持该格式」可判定档位：按 magic 区分「不是 zip 容器」与「zip 但损坏」，
  前端补档位 + 三语。缺陷面见 #348。
- **T21-B** 安全外壳与格式解耦，**验收载体是 rar**：条目数/单文件/总解压上限、`..` 逃逸拒绝、
  symlink 条目拒绝、task-scoped sandbox 目前都长在 zip reader 里。
  rar 破掉两个隐含假设——**条目总数事先不可知**（`RARReadHeaderEx` 逐条顺序读，
  预检拒绝降级为中途中止）、**条目类型不止文件/目录**（RAR5 的 symlink / hardlink 是一等条目）。
  接口硬要求：声明大小允许「未知」、**配额强制在字节流层而非元数据层**、
  **超限能立即中止而不是读完再判**。zip / rar / 7z 都落进这一套，无需为任何一种单开配额档。
- **T21-C** rar：**已拍板做，自行 vendor 官方 UnRAR 7.23 源码 ＋ 写 FFI**（仅需 6 个导出），
  不用现成 crate（其 vendor 的是 7.21 beta 1，且 `read()` 抽象强制全量入内存）。
  许可已核对原文：它禁的是开发 RAR 兼容**压缩器**与重建**压缩**算法，我们只解压，不触及；
  再分发允许，条件是第 2 条全文进 license（本仓库无 LICENSE，走 documentation → `NOTICE.md`）
  与源码注释；无商业限制。**vendor 目录须原样保留 `license.txt`。**
  两条硬前置：① **unrar 全程不得碰文件系统**——`RAR_TEST` ＋ `UCM_PROCESSDATA` 回调流式收字节，
  落盘位置由 B 的外壳决定；② 接受跟踪上游安全公告的长期义务（unrar 有 CVE-2012-6706、
  2017-12940/12941/12942、2026-24857 等多条 CRITICAL 内存安全问题，
  以及 CVE-2017-12938、2022-30333、2022-48579 三条 symlink/遍历绕过——最后这类正由前置①消除）。
- **T21-D** 7z：纯 Rust，无 C 依赖；solid block 整块解压，与 rar 同类，B 做对了这里是复用。
- **T21-E** tar 家族：`out_of_scope`（本轮）。分析留在 `TODO.md`，恢复时直接用。

完成定义（除通用切片定义外）：

- A：`.rar` 用例给出可操作提示；**截断的 zip 仍报「请检查压缩包后重试」**（新档位不能吃掉它）；
  反向验证——去掉 magic 判别后 `.rar` 必须退回 `retry-hint`。
- B：rar 与 zip 共用同一组负测（解压炸弹 / 路径逃逸 / symlink 条目 / 条目数超限），
  **一条都不为 rar 单独重写。要重写就说明外壳没抽干净。**
- C：另加 hardlink 条目、分卷、加密包各自落到明确档位；`NOTICE.md` 已补第 2 条全文与第 4 条免责。
- D：同上一组负测全部复用通过。

先于 B 需核实的一条（**只有代码阅读，无实测，不得当成结论**）：上限判据用的是 zip 头里的**声明**
大小，实际写盘的 `copy_with_cancellation` 读到 EOF 为止、无字节上限。造一个声明大小远小于实际
解压量的 zip，量沙箱落盘字节数即可判定。若成立，先修它再谈新格式。

提交边界：后端档位与错误码一个提交，前端档位与三语文案一个提交，安全外壳重构单独一个提交，
每个新格式各自一个提交。

### T22：拖拽单个 / 多个 Mod 包到窗口批量导入

状态：`blocked`——**批量编排的架构取舍未定**（复用 T17 来源 adapter 一般化 vs 新写第二套批量语义）。
这属「维护者选择未定义的产品策略」，定了之后 T22-1 转 `ready`。立案 #366，前置 T21-A。
**2026-09-08 定序：排在 T21-D（7z）之后。**

核心不是拖放本身，是**拖入后的待导入清单**：逐行勾选、L1 容器层即时分档、L2 内容层异步预检
（复用 #354 的包内容只读查询）。

本文件特有的门禁映射：

- **不因为拖放就自动安装或启用**——与 T17 的 import-only 边界同一条，不得借拖放绕过
  `InstallPlan / manifest / backup / rollback`。
- **「看起来不是本游戏的 Mod」只能警示、不能否决**，且判据必须是结构性的（有没有可识别内容根），
  **不得引入词表**（「文件名带 setup 就拒绝」这类规则等于不工作）。容器层三档（格式不支持 /
  不是压缩包 / 损坏）是硬事实，不给覆盖。
- 预检只读：不写库、不写游戏目录；读失败降级成「未知，需你确认」，不得判成不可导入。
- 拖入路径不进日志、不进用户可见文案；清单显示文件名而非完整路径。

完成定义（除通用切片定义外）：混合素材一次拖入（正常 zip ＋ `.rar` ＋ `.exe` ＋ 截断 zip ＋ 重复包）
五行各归其档、无一被静默丢弃；取消勾选的包不产生任何任务事实；关闭浮层零入库；
「看起来不是 Mod」手动勾上能正常导入；反向验证——内容层判据改成恒真时该档位必须消失。
...
- 新手引导的后续页面内业务步骤由维护者显式纳入，不得由无人值守队列自行扩张；Phase 1 状态、门槛和后续交互范围以
