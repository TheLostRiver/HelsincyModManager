...
仓库只对 `.codex/skills/hmm*` 下的 HMM 项目技能进行版本管理。其他 Codex hooks、通用 skills、
脚本、模板和个人配置属于本地工具状态，由 `.gitignore` 排除。

仓库内的 `.agents/` 目录用于存放 Antigravity IDE、Gemini 或其他外部 agent 可读取的协作规则适配文件。它不是运行时缓存，而是项目治理文档的一部分。

约束：

- 只有 `.codex/skills/hmm*` 下的项目技能及其 references / agent metadata 可以纳入版本管理。
- `.codex/` 下其他 hooks、通用 skills、脚本、模板和个人配置不得加入 Git。
...
- 修改受版本管理的 `.codex/skills/hmm*` 视为治理相关变更，应触发人工 review。
